...
void usage(const std::string& msg) {
  std::cout << "Usage: " + msg << " -i inputfile [-t maxthreads (16)]" << std::endl;
}
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "================================================================" << std::endl;
  std::cout << "  Compressed sparse row OMP static scheduling" << std::endl;


  omp_set_schedule(omp_sched_static, blocksize);
  ...
  plt::named_loglog("Static", sizes, flops);


  std::cout << "================================================================" << std::endl;
  std::cout << "  Compressed sparse row OMP dynamic scheduling" << std::endl;


  omp_set_schedule(omp_sched_dynamic, blocksize);

  sparse_matvec_bench<CSRMatrix, Vector>
    ([](const CSRMatrix& A, const Vector& x, Vector& y) { A.omp_matvec(x,y); },
     sizes, flops, maxthreads, input_file);

  plt::named_loglog("Dynamic", sizes, flops);


  std::cout << "================================================================" << std::endl;
  std::cout << "  Compressed sparse row OMP guided scheduling" << std::endl;

  omp_set_schedule(omp_sched_guided, blocksize);

  sparse_matvec_bench<CSRMatrix, Vector>
    ([](const CSRMatrix& A, const Vector& x, Vector& y) { A.omp_matvec(x,y); },
     sizes, flops, maxthreads, input_file);

  plt::named_loglog("Guided", sizes, flops);


  std::cout << "================================================================" << std::endl;
  std::cout << "  Compressed sparse row OMP auto scheduling" << std::endl;

  omp_set_schedule(omp_sched_auto, blocksize);

  sparse_matvec_bench<CSRMatrix, Vector>
    ([](const CSRMatrix& A, const Vector& x, Vector& y) { A.omp_matvec(x,y); },
     sizes, flops, maxthreads, input_file);

  plt::named_loglog("Auto", sizes, flops);

  plt::xlim(1, (int) maxthreads);
  plt::xlabel("Threads");
  plt::ylabel("GFlops");
  plt::title("Sparse Matrix Comparison of Scheduling " + input_file);
  plt::legend();
  plt::save("csr_omp.png");
