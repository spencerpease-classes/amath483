...
int main(int argc, char* argv[]) {
  ...
  for (size_t size = 16; size <= maxsize; size *= 2) {
    ...
    RowMatrix A(size, size), B(size, size), C(size, size);
  ...
  std::cout << "================================================================" << std::endl;
  std::cout << "  Column ordered " << std::endl;
  std::cout << "N\ttrips\ttotal\tavg\tGflops" << std::endl;
  sizes.clear();
  flops.clear();
  for (size_t size = 16; size <= maxsize; size *= 2) {
    size_t trips = 2 +  1024 * 1024 * 1024 / (size * size * size);
    ColMatrix A(size, size), B(size, size), C(size, size);
    randomize(A); randomize(B); randomize(C);
  ...
  plt::named_loglog("Column", sizes, flops);
